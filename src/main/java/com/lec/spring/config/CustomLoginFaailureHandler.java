...
public class CustomLoginFaailureHandler implements AuthenticationFailureHandler {
    private final String DEFAULT_FAILURE_FORWARD_URL = "/user/loginError";

    // 인증 실패 직후 호출되는 콜백
    @Override
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        System.out.println("❤️❤️ 로그인 실패  : onAuthenticationFailure 호출");
        ...
        // 서버 내부로 이동, url 주소가 바뀌지 않음
        request.getRequestDispatcher(DEFAULT_FAILURE_FORWARD_URL).forward(request, response);  // **** 이해 안됨
    }
}
